#!/usr/bin/env python3
"""Run Fuzzy PID and PID simulations sequentially across multiple scenarios and seeds."""
...
def run_simulation(
    ...
    processes: list[subprocess.Popen[bytes]] = []
    print(f"\n{'=' * 68}\nStarting {name} | Scenario: {scenario} | Seed: {seed}\n{'=' * 68}", flush=True)

    try:
        scenario_file = WORKSPACE / "src" / "car_controller" / "config" / "simulation_scenarios" / f"{scenario}.yaml"
        if not scenario_file.is_file():
            raise FileNotFoundError(f"Scenario file not found: {scenario_file}")

        command = (
            "ros2 launch car_controller prius_sonoma_control.launch.py "
...
def indexed_path(path: Path, scenario: str, seed: int) -> Path:
    return path.with_name(f"{path.stem}_{scenario}_{seed}{path.suffix}")


def archive_log(source: Path, scenario: str, seed: int) -> Path:
    if not source.is_file():
        raise RuntimeError(f"Simulation did not create the expected log: {source}")
    destination = indexed_path(source, scenario, seed)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run fuzzy PID and PID simulations across scenarios and seeds."
    )
    parser.add_argument(
        "--scenario",
        type=str,
        default="baseline",
        help="Scenario to run (e.g. baseline, grade_step, sensor_noise, combined). Default: baseline",
    )
    parser.add_argument(
        "--seeds",
        type=int,
        nargs="+",
        default=[1001],
        help="List of random seeds to run (default: 1001)",
    )
    parser.add_argument(
        "--duration",
        type=float,
        default=46.0,
        help="Target simulation time in seconds (default: 27.0)",
    )
    parser.add_argument(
        "--startup-timeout",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=120.0,
        help="Maximum real seconds to wait for the simulation duration to complete (default: 120.0)",
    )
...
def main() -> int:
    ...
    try:
        for idx, seed in enumerate(args.seeds, 1):
            print(f"\n######## Seed pair {idx}/{len(args.seeds)} (Seed: {seed}) ########", flush=True)
            for name, controller, log_path in CONTROLLERS:
                ...
                archived_logs.append(archive_log(log_path, args.scenario, seed))
    ...
    print(f"\nCompleted {len(args.seeds)} simulation pair(s) successfully for scenario '{args.scenario}'.", flush=True)
    for path in archived_logs:
        print(path)
    
    seeds_str = " ".join(map(str, args.seeds))
    print(f"\nCreate plots with: python3 logs/plot_velocity_log.py --scenario {args.scenario} --seeds {seeds_str}")
    return 0
